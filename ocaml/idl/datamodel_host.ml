...
*)
  ...
  let call_plugin = call
      ~name:"call_plugin"
      ~in_product_since:rel_orlando
      ~doc:"Call a management API plugin on this host"
      ~params:[Ref _host, "host", "The host";
  ...
  let call_extension = call
      ...
      ~in_product_since:rel_ely
      ~custom_marshaller:true
      ~doc:"Call a management API extension on this host"
      ~params:[Ref _host, "host", "The host";
  ...
  let set_ssl_legacy = call
      ...
      ~doc:"Enable/disable SSLv3 for interoperability with older versions of XenServer. When this is set to a different value, the host immediately restarts its SSL/TLS listening service; typically this takes less than a second but existing connections to it will be broken. Management API login sessions will remain valid."
      ~params:[
        Ref _host, "self", "The host";
    ...
    create_obj ~in_db:true ~in_product_since:rel_rio ~in_oss_since:oss_since_303 ~internal_deprecated_since:None ~persist:PersistEverything ~gen_constructor_destructor:false ~name:_host ~descr:"A physical host" ~gen_events:true
      ...
      ~contents:
        ([ uid _host;
           ...
           field ~qualifier:StaticRO ~lifecycle:[Published, rel_dundee, ""] ~ty:Bool ~default_value:(Some (VBool true)) "ssl_legacy" "Allow SSLv3 protocol and ciphersuites as used by older XenServers. This controls both incoming and outgoing connections. When this is set to a different value, the host immediately restarts its SSL/TLS listening service; typically this takes less than a second but existing connections to it will be broken. Management API login sessions will remain valid.";
