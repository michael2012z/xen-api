...
module Sr_stat = struct
  ...
  let t =
    ...
    create_obj
      ...
      ~contents:
        [
          field ~qualifier:DynamicRO ~lifecycle ~ty:(Option String) "uuid" "Uuid that uniquely identifies this SR, if one is available.";
...
module VDI = struct
  ...
  let open_database = call
      ...
      ~doc:"Load the metadata found on the supplied VDI and return a session reference which can be used in management API calls to query its contents."
      ~allowed_roles:_R_POOL_OP
      ()
