...
*)
  ...
  let set_is_default_template = call
      ...
      ~hide_from_docs:true
      ~lifecycle: [Published, rel_falcon, "Allows to define Citrix Hypervisor default templates"]
    ...
    create_obj ~in_db:true ~in_product_since:rel_rio ~in_oss_since:oss_since_303 ~internal_deprecated_since:None ~persist:PersistEverything ~gen_constructor_destructor:true ~name:_vm ~descr:"A virtual machine (or 'guest')."
      ...
      ~contents:
        ([ uid _vm;
           ...
           field ~ty:Bool ~default_value:(Some (VBool false)) ~qualifier:DynamicRO ~writer_roles:_R_POOL_ADMIN ~lifecycle:[Published, rel_falcon, "Identifies Citrix Hypervisor default templates"] "is_default_template" "true if this is a default template. Default template VMs can never be started or migrated, they are used only for cloning other VMs";
